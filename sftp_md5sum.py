...
def run_sftp_md5sum(name_remote, name_local):
    ...
    md5_dict_1 = calculate_md5_md5sum_batch(
        dir_local_1, pb_desc=f"{name_local}|md5sum|1"
    )
    md5_dict_2 = calculate_md5_md5sum_batch(
        dir_local_2, pb_desc=f"{name_local}|md5sum|2"
    )
    write_md5_from_dict(md5_dict_1, path_md5sum_1)
    write_md5_from_dict(md5_dict_2, path_md5sum_2)
    idx = compare_md5(path_md5sum_1, path_md5sum_2, output_dir)
    if idx:
        file_done = os.path.join(dir_log, "done", "pass", name_local)
    else:
        file_done = os.path.join(dir_log, "done", "fail", name_local)
    os.makedirs(os.path.dirname(file_done), exist_ok=True)
    with open(file_done, "w") as f:
        pass
...
def main():
    parser = argparse.ArgumentParser(
        description="Download files from SFTP server and compare MD5 checksums."
    )
    parser.add_argument("name_remote", type=str, help="Remote directory name")
    parser.add_argument("name_local", type=str, help="Local directory name")

    args = parser.parse_args()
    run_sftp_md5sum(args.name_remote, args.name_local)
